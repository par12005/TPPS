...
function tpps_page_2_validate_form(array &$form, array &$form_state) {
  if ($form_state['submitted'] == '1') {
    ...
    if (isset($form_state['values']['study_info']['assessions']) and !$form_state['values']['study_info']['assessions']) {
      form_set_error('study_info][assessions', t('Number of times the populations were assessed: field is required.'));
    }
    ...
    if (!empty($form_state['values']['study_info']['rooting'])) {
      ...
      $ph = $root['ph'];
      if (!$ph['option']) {
        form_set_error("study_info][rooting][ph][option", t("pH controlled or uncontrolled: field is required."));
      }
      elseif ($ph['option'] == '1' and !$ph['controlled']) {
        form_set_error("study_info][rooting][ph][controlled", t("Controlled pH Value: field is required."));
      }
      elseif ($ph['option'] == '2' and array_key_exists('uncontrolled', $ph) and !$ph['uncontrolled']) {
        form_set_error("study_info][rooting][ph][uncontrolled", t("Average pH Value: field is required."));
      }
      ...
      foreach ($root['treatment'] as $field => $value) {
        ...
        elseif ($selected and !$value) {
          form_set_error("study_info][rooting][treatment][$field", t("@field: field is required.", array('@field' => $field)));
        }
        $description = FALSE;
      }
    }
    ...
    if (!empty($form_state['values']['study_info']['irrigation'])) {
      ...
      if (!$irrigation['option']) {
        form_set_error('study_info][irrigation][option', t('Irrigation Type: field is required.'));
      }
      elseif ($irrigation['option'] == 'Other' and !$irrigation['other']) {
        form_set_error('study_info][irrigation][other', t('Custom Irrigation Type: field is required.'));
      }
    }

    if (!empty($form_state['values']['study_info']['biotic_env']) and preg_match('/^0+$/', implode('', $form_state['values']['study_info']['biotic_env']['option']))) {
      form_set_error('study_info][biotic_env', t('Biotic Environment: field is required.'));
    }
    elseif (!empty($form_state['values']['study_info']['biotic_env']['option']['Other']) and !$form_state['values']['study_info']['biotic_env']['other']) {
      form_set_error('study_info][biotic_env][other', t('Custom Biotic Environment: field is required.'));
    }
    ...
    if (!empty($form_state['values']['study_info']['treatment']) and $form_state['values']['study_info']['treatment']['check']) {
      ...
      foreach ($treatment as $field => $value) {
        if ($field != 'check') {
          ...
          elseif ($selected and !$value) {
            form_set_error("study_info][treatment][$field", t("@field: field is required.", array('@field' => $field)));
          }
          $description = FALSE;
        }
      }
      ...
      if ($treatment_empty) {
        form_set_error("study_info][treatment", t('Treatment: field is required.'));
      }
    }
  }
}
